...
public class CacheKeyGenerator implements KeyGenerator
{
	...
	public Class<?> getProxyClass(Object target) {
		...
		if (target instanceof Proxy) {
			...
			if (invo.getClass().toString().contains("MapperProxy")) {
				clazz = (Class) ReflectExtraUtils.getFieldValue(invo, "mapperInterface");
				return clazz;
			} else if (invo.getClass().toString().contains("JdkDynamicAopProxy")) {
				ProxyFactory advised = (ProxyFactory)ReflectExtraUtils.getFieldValue(invo, "advised");
